% change in parameters and few syntaxes


function [bfSIG,M] = ezdas(SIG,x,y,z,vsource,param) 
...
dTX = hypot(hypot(x - x0, z - z0), y - y0) ...
     - hypot(hypot((abs(x0)-L/2).*(abs(x0)>L/2), z0), y0);

% 2) RECEIVE DISTANCE: voxel -> each array element
% dRX(m) = sqrt( (x - xe(m))^2 + z^2 + y^2)

dRX = hypot(hypot(x - xe, z), y);
