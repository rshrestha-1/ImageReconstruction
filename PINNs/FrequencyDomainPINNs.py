...
# Sensor indices
sensor_vals = np.arange(num_sensors)

# Normalise to [-1,1]
sensor_vals = 2 * (sensor_vals / (num_sensors - 1)) - 1
...
class PINN(nn.Module):
    ...
    def forward(self, x, y, z, s, f):
        inputs = torch.stack([x, y, z, s, f], dim=1)
        out = self.net(inputs)
        return out[:,0], out[:,1]
...
for epoch in range(3000):
    ...
    # Random sampling
    s_idx = torch.randint(0, num_sensors, (batch_size,))
...
coords_new[:,0] = 2 * (coords_new[:,0] / (Nx_new - 1)) - 1
coords_new[:,1] = 2 * (coords_new[:,1] / (Ny_new - 1)) - 1
coords_new[:,2] = 2 * (coords_new[:,2] / (Nz_new - 1)) - 1
