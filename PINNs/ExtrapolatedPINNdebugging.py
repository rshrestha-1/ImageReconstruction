...
batch_size_data = 8000
batch_size_phys = 1000
loss_total_hist = []
loss_data_hist = []
loss_phys_hist = []
...
for epoch in range(3000):
    ...
    # Random sampling data batch
    s_idx = torch.randint(0, len(sensor_vals), (batch_size_data,))
    f_idx = torch.randint(0, K, (batch_size_data,))
    v_idx = torch.randint(0, N, (batch_size_data,))

    # Random sampling physics batch
    s_idx_p = torch.randint(0, len(sensor_vals), (batch_size_phys,))
    f_idx_p = torch.randint(0, K, (batch_size_phys,))
    v_idx_p = torch.randint(0, N, (batch_size_phys,))
    ...
    # Clone inputs for physics batch
    x_p = coords[v_idx_p][:,0].clone().detach().requires_grad_(True)
    y_p = coords[v_idx_p][:,1].clone().detach().requires_grad_(True)
    z_p = coords[v_idx_p][:,2].clone().detach().requires_grad_(True)

    s_p = sensor_vals[s_idx_p]
    f_p = f_norm[f_idx_p]
    k_p = k_vals[f_idx_p]
    ...
    # Physics loss
    loss_p = physics_loss(model, x_p, y_p, z_p, s_p, f_p, k_p)

    loss = loss_d + 1e-4 * loss_p
    ...
    if epoch % 200 == 0:
        ...
        print("RAM GB:", psutil.Process(os.getpid()).memory_info().rss / 1e9)
    del loss, loss_d, loss_p
...
# Centered and normalised coordinates: map new 10 mm cube to [-1,1] while keeping center at 0
coords_new[:,0] = 2 * (coords_new[:,0] / (Nx_new - 1) - 0.5) * (phys_size_new / phys_size_orig)
coords_new[:,1] = 2 * (coords_new[:,1] / (Ny_new - 1) - 0.5) * (phys_size_new / phys_size_orig)
coords_new[:,2] = 2 * (coords_new[:,2] / (Nz_new - 1) - 0.5) * (phys_size_new / phys_size_orig)
...
savemat('H_extrapolated_time.mat', {
    ...
    'loss_data': np.array(loss_data_hist),
    'loss_physics': np.array(loss_phys_hist)
})
